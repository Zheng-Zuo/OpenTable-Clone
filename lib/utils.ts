...
import { Filters } from "./actions/search.actions"
...
export function filterToString(filters: Filters) {
...
export const stringToFilter = (str: string): Filters => {
  ...
  decodeURIComponent(str).split('+').forEach((part) => {
    ...
    filters[key as keyof Filters] = valuesArray
  });
...
export const calculateReviewRatingAverage = (reviews: Review[]) => {
    ...
    const avg_rating = reviews.reduce((total, review)=>{
